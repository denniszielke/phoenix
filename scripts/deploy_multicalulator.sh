...
REDIS_HOST=$(az keyvault secret show --name "phoenix-redis-host" --vault-name $AZURE_KEYVAULT_NAME)
REDIS_AUTH=$(az keyvault secret show --name "phoenix-redis-access" --vault-name $AZURE_KEYVAULT_NAME)
APPINSIGHTS_KEY=$(az keyvault secret show --name "phoenix-appinsights-key" --vault-name $AZURE_KEYVAULT_NAME)
